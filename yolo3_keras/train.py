...
"""
import yolo3_keras.train_model_config as train_model_config
from yolo3_keras.yolo3.model import preprocess_true_boxes, yolo_body, tiny_yolo_body, yolo_loss
from yolo3_keras.yolo3.utils import get_random_data
...
def start_training(is_retrain:bool, annotation_path:str,
    ...
    if is_retrain:
        ...
        if is_tiny_version:
            ...
            model = create_tiny_model(
                ...
                weights_path = os.path.join(model_dir_path,load_model_name))
        else:
            model = create_model(
                input_shape, anchors, num_classes, freeze_body=2,
                weights_path = os.path.join(model_dir_path,load_model_name))
    ...
    model.fit(
        ...
        epochs = epochs,
        callbacks = train_model_config.get_callbacks(log_dir=model_dir_path),
    )

    for i in range(len(model.layers)):
        model.layers[i].trainable = False
    model.save_weights(os.path.join(model_dir_path,save_model_name))
